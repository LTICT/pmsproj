...
public function supersetData(Request $request){
   $authenticatedUser = $request->authUser;
   $userId=$authenticatedUser->usr_id;
   $query="SELECT r.rol_description AS superset_url 
FROM tbl_roles r
INNER JOIN tbl_user_role ON r.rol_id = tbl_user_role.url_role_id 
WHERE tbl_user_role.url_user_id = ".$userId." 
AND r.rol_description LIKE '%superset%' ";
$data_info=DB::select($query);
    $resultObject= array(
    "data" =>$data_info,
