...
use App\Models\Modelpmsprojectevaluation;
use Illuminate\Http\Request;
use Illuminate\Support\Facades\Validator;
use Illuminate\Support\Facades\DB;
//PROPERTY OF LT ICT SOLUTION PLC
class PmsprojectevaluationController extends MyController
{
 public function __construct()
 {
    ...
    {
        $query='SELECT mne_id,mne_transaction_type_id,mne_visit_type,mne_project_id,mne_type_id,mne_physical,mne_financial,mne_physical_region,mne_financial_region,mne_team_members,mne_feedback,mne_weakness,mne_challenges,mne_recommendations,mne_purpose,mne_record_date,mne_start_date,mne_end_date,mne_description,mne_create_time,mne_update_time,mne_delete_time,mne_created_by,mne_status FROM pms_project_evaluation ';
        $query .=' WHERE mne_id='.$id.' ';
        $data_info=DB::select(DB::raw($query));
        if(isset($data_info) && !empty($data_info)){
            $data['pms_project_evaluation_data']=$data_info[0];
        }
        $data['page_title']=trans("form_lang.pms_project_evaluation");
        return view('project_monitoring_evaluation.show_pms_project_evaluation', $data);
    }
    //Get List
    public function listgrid(Request $request){     
       $query="SELECT mne_physical_zone,mne_financial_zone,mne_strength, prj_name,prj_code,mne_id,mne_transaction_type_id,mne_visit_type,mne_project_id,mne_type_id,mne_physical,mne_financial,mne_physical_region,mne_financial_region,mne_team_members,mne_feedback,mne_weakness,mne_challenges,mne_recommendations,mne_purpose,mne_record_date,mne_start_date,mne_end_date,mne_description,mne_create_time,mne_update_time,mne_delete_time,mne_created_by,mne_status,1 AS is_editable, 1 AS is_deletable FROM pms_project_evaluation
       INNER JOIN pms_project ON pms_project.prj_id=pms_project_evaluation.mne_project_id ";
       $query .=' WHERE 1=1';
...
public function updategrid(Request $request)
{
    ...
    if($validator->fails()) {
        ...
        if(isset($id) && !empty($id)){
            $data_info = Modelpmsprojectevaluation::findOrFail($id);
        ...
        $data_info=Modelpmsprojectevaluation::create($requestData);
        $resultObject= array(
            "odata.metadata"=>"",
            "value" =>$data_info,
            "statusCode"=>200,
...
{
    $id=$request->get("mne_id");
    Modelpmsprojectevaluation::destroy($id);
    $resultObject= array(
        "odata.metadata"=>"",
        "value" =>"",
        "statusCode"=>200,
...
function listRoutes(){
    Route::resource('project_evaluation', 'PmsprojectevaluationController');
